import { cloneElement, forwardRef, isValidElement, type ReactElement, type ReactNode, useEffect, useImperativeHandle, useRef } from "react";
import { useTimelineContext } from "../core";
import { resolveTarget, safeJsonStringify } from "../core/helpers";
import type { AnimatableProperties, AnimationTargets, PlaybackSettings, TimelineEntry as TimelineEntryType, TweenParameters } from "../types";
...
export const TimelineEntry = forwardRef<TimelineEntryRef, TimelineEntryProps>(function TimelineEntry(
  ...
  const resolvedTargetsRef = useRef<HTMLElement | SVGElement | NodeList | (HTMLElement | SVGElement)[] | null>(null);
  ...
  if (children) {
    if (!isValidElement(children)) {
      console.warn("[react-animejs] TimelineEntry requires a single valid React element as child");
