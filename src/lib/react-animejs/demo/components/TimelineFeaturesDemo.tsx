...
export const TimelineFeaturesDemo: React.FC = () => {
  ...
  const { controls, state } = useAnimeTimeline(
    ...
    [
      // 1. Add a label at the start
      { label: 'start' },
      
      // 2. Add first animation
      { 
        targets: box1Ref, 
        translateX: 200, 
        duration: 1000, 
      ...
      // 3. Add a function call after first animation
      { 
        callback: () => setLastEvent('First Animation Done'), 
        position: '+=0' 
      },
      
      // 4. Add second animation starting at the same time as first ('<')
      { 
        targets: box2Ref, 
        translateX: 200, 
        duration: 1000, 
      ...
      // 5. Add a timer (no target) for a pause
      { 
        duration: 500, 
        position: '+=' 
      },
      
      // 6. Add third animation after the pause
      { 
        targets: box3Ref, 
        translateX: 200, 
        duration: 1000, 
      ...
      // 7. Add a function call at a specific label
      { 
