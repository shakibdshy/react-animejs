...
/**
 * The stack of cards, driven by a single shared pin progress (0→1). Extracted
 * so <AnimeScroll> can re-render it via its render prop without re-running the
 * pin setup. At most TWO cards are ever visible — the current one and the one
 * covering it.
 */
function CardStack({ progress }: { progress: number }) {
  const last = CARDS.length - 1;
  return (
    // perspective on this wrapper gives the cards real depth without
    // establishing a containing block for the pinned <section> above it.
    <div className="relative h-[70vh] w-full max-w-3xl" style={{ perspective: '1200px' }}>
      {CARDS.map((card, i) => {
        // The stack is split into (N-1) transitions, one per scroll slice.
        // Transition k covers progress slice [k, k+1]/(N-1): card k recedes
        // while card k+1 rises to cover it.
...
export function StackedCardsDemo() {
  ...
  const endSpacing = enabled ? last * (typeof window !== 'undefined' ? window.innerHeight : 800) : 0;
    ...
    <div className="relative">
      ...
      */}
      <main className="overflow-x-hidden">
        {/*
          <AnimeScroll> in pin mode owns the pinned container: the render prop
          hands us the ref to attach, plus progress / isPinned. One pin drives
          the whole stack — the container stays fixed across (N-1) viewport
          heights of scroll, and each card derives its recede from a slice of
          that shared progress.
        */}
