...
const generateMorphPoints = (count: number) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    points.push(
      `${Math.round(128 + Math.cos(angle) * radius)} ${Math.round(192 + Math.sin(angle) * radius)}`
    );
  }
  return points.join(' ');
};
              ...
              {DRAWABLE_SHAPES.map((shape, index) => (
                ...
                >
                  {shape.type === 'path' ? (
                    ...
                    <polyline points={shape.points} data-testid={`draw-line-${index}`} />
                  )}
                </AnimeDraw>
              ))}
            </g>
          ...
          >
            ...
            <AnimeMotionPath path={trackRef} duration={5000} ease="linear" loop autoplay>
              <g data-testid="motion-path-car">
                <rect x="-9" y="-5" width="18" height="10" rx="3" fill="#f8fafc" />
                <rect x="-3" y="-8" width="10" height="6" rx="2" fill="#38bdf8" />
