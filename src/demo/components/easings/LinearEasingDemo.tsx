...
import { linear } from '@/lib/react-animejs';
import { RotateCcw } from 'lucide-react';

const linearPresets = [
  { name: 'smooth', values: [0, 0.5, 0.5, 1] as number[] },
  { name: 'bounce-like', values: [0, 0, 0.5, 0.5, 1, 1] as number[] },
  { name: 'overshoot', values: [0, '1 25%', 0, 1] as (number | string)[] },
  { name: 'stutter', values: [0, 0.3, 0.3, 0.7, 0.7, 1] as number[] },
];

export const LinearEasingDemo: React.FC = () => {
  const ref0 = useRef<HTMLDivElement>(null);
  const ref1 = useRef<HTMLDivElement>(null);
  const boxRefs = useMemo(() => [ref0, ref1], []);
  const [presetIndex, setPresetIndex] = useState(0);
  const preset = linearPresets[presetIndex];
  const easing = useMemo(
    () => linear(...(preset.values as [number, ...Array<number>])),
    [presetIndex]
  );
  ...
  return (
    <div className="w-full bg-demo-card rounded-3xl p-6 border border-demo-border shadow-xl">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h4 className="text-demo-accent font-bold text-xl lowercase">linear</h4>
          <p className="text-xs text-demo-text-muted mt-1">Multi-stop linear interpolation</p>
        </div>
        ...
        <div className="flex flex-col gap-4 w-full">
          {boxRefs.map((ref, i) => (
            <div key={i} className="flex items-center gap-4">
              ...
              <div
                ref={ref}
                className="w-10 h-10 bg-[#6366f1] rounded-full shadow-[0_0_15px_rgba(99,102,241,0.2)]"
              />
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 text-[10px] text-demo-text-secondary font-mono bg-black/30 p-2.5 rounded-lg border border-demo-border overflow-x-auto">
        <code className="text-demo-accent/80">linear({preset.values.join(', ')})</code>
      </div>
    </div>
  );
};
