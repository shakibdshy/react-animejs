...
export const ScopeMediaQueriesDemo: React.FC = () => {
  ...
  const { ref, matches } = useAnimeScope({ mediaQueries: MEDIA_QUERIES });
  ...
  return (
    <DemoCard
      title="Scope"
description="Using useAnimeScope and useAnime hooks together. The animation automatically adapts when the container crosses 200px."
code={`useAnimeScope({ mediaQueries: { isSmall: '(max-width: 200px)' } })\nuseAnime({ x: matches.isSmall ? 0 : 100, duration: matches.isSmall ? 0 : 750 })`}
    >
