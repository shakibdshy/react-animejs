...
import type { DemoId, FilterCategory } from './data';
...
export function useDetailOverlay(filteredDemos: readonly DemoSection<DemoId>[]) {
  const [isOpen, setIsOpen] = useState(false);
  const [currentIndex, setCurrentIndex] = useState(-1);
  const [activeDemo, setActiveDemo] = useState<DemoSection<DemoId> | null>(null);
