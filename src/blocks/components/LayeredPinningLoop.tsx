 ...
 * "Layered pinning with infinite looping" ScrollTrigger demo.
 *
...
import { Anime, useAnimeOnScroll, utils } from '@/lib/react-animejs';
...
const PANELS: LoopPanel[] = [
  ...
  },
  {
    ...
    description: 'A focused stage, deliberate contrast, and a little breathing room for the motion.',
    accent: '#64d5ff',
    surface: '#0d161b',
  },
  {
    ...
    description: 'A steady scroll rhythm lets each transition land without forcing the browser to jump.',
    accent: '#b29cff',
    surface: '#131020',
  },
  {
    ...
    description: 'The finite sequence ends cleanly after the final layer, ready for the next block on the page.',
    accent: '#73e0a6',
    surface: '#0e1713',
  },
];
...
export const LayeredPinningLoop = memo(function LayeredPinningLoop({
  ...
  const { ref: trackRef, progress } = useAnimeOnScroll<HTMLDivElement, HTMLDivElement>({
          ...
          {PANELS.map((panel, index) => {
            return (
              ...
              >
                ...
                />
                <div
                  className="relative mx-auto flex h-full w-full max-w-5xl flex-col justify-between px-7 py-8 transition-all duration-500 sm:px-12 sm:py-12"
                >
        ...
        <div className="flex items-center gap-3">
          <div className="h-1 w-28 overflow-hidden rounded-full bg-white/10">
            <div
              ...
              style={{ width: `${Math.round(normalizedProgress * 100)}%`, backgroundColor: activePanel.accent }}
            />
          </div>
