 ...
 *
 * Toggling `easeReverse` ON allows each element to retract/fade with smooth,
 * non-bouncy curves (like outQuad/outCubic) at a customizable exit speed multiplier,
 * instead of reversing the bouncy entry curves (outBack).
 */
import {
  memo,
...
import { AnimeProvider, stagger, useAnimeTimeline } from '@/lib/react-animejs';
...
const OrchestratedEaseReverseInner = memo(function OrchestratedEaseReverseInner({
  ...
  // 1. Entrance / Forward Timeline (Bouncy entry)
  const openTimeline = useAnimeTimeline(
    {
      autoplay: false,
      onComplete: () => {
  ...
  // 2. Custom Exit Timeline (Smooth exit, active when easeReverse is ON)
  const closeTimeline = useAnimeTimeline(
    {
      autoplay: false,
      onComplete: () => {
        setIsOverlayVisible(false);
    ...
    [
      ...
      },
      {
        targets: '.er-menu-btn',
        opacity: 1,
        duration: 150 / exitSpeed,
        ease: 'outQuad',
        position: 650 / exitSpeed,
      },
      {
        targets: '.er-backdrop',
        opacity: 0,
        duration: 300 / exitSpeed,
        ease: 'outQuad',
        position: 500 / exitSpeed,
      },
      {
        targets: '.er-menu-panel',
        opacity: 0,
  ...
  const toggleMenu = useCallback(() => {
    setIsOpen((prev) => {
      const next = !prev;
      const baseDuration = 900; // openTimeline length is ~900ms

      if (next) {
        // OPENING
        setIsOverlayVisible(true);
        closeTimeline.controls.pause();

        if (openTimeline.state.reversed) {
          openTimeline.controls.reverse();
        }
        openTimeline.controls.stretch(baseDuration);
        openTimeline.controls.restart();
      } else {
        // CLOSING
        if (easeReverse) {
          // Custom smooth retract
          openTimeline.controls.pause();
          closeTimeline.controls.restart();
        } else {
          // Bouncy backwards retract (Standard reverse)
          closeTimeline.controls.pause();
          if (!openTimeline.state.reversed) {
            openTimeline.controls.reverse();
          }
          openTimeline.controls.stretch(baseDuration / exitSpeed);
          openTimeline.controls.play();
        }
      }
      return next;
    });
    ...
    easeReverse,
    exitSpeed,
    openTimeline.controls,
    openTimeline.state.reversed,
    closeTimeline.controls,
  ]);
